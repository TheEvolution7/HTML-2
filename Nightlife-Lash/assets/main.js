...
"use strict";
  ...
  $('.testimonial__nav').slick({
	...
	////////////////////////////////////////////////////
	// 10. Product Slider Js
	var owl = $('.question-slide');
	owl.owlCarousel( {
		loop:false,
		items:1,
		nav:true,
		navText:['<button type="button" class="os-btn os-btn-white">Prev</button>','<button type="button" class="os-btn os-btn-white">Next</button>'],
		autoHeight: true,
		mouseDrag: false
	});
	$('#btn-started').on('click', function(){
		$('#started').hide();
		$('#form-q').show();
	})
	// Listen to owl events:
	owl.on('changed.owl.carousel', function(event) {
		var a    = event.item.count;     // Number of items
		var b     = event.item.index; 
		var c = a - 1 
		console.log(c)
		if (b == c){
			$('.owl-next button', this).delay(100).queue(function(next) { $(this).attr('type', 'submit'); next(); })
			// $('.owl-next button', this).attr('type', 'submit')
			$('.owl-next button', this).html('Submit');
		} 
		else {
			$('.owl-next button', this).attr('type', 'button');
			$('.owl-next button', this).html('Next');
		}
	...
	function moved() {
		...
		var owl = $(".question-slide").data('owlCarousel');
