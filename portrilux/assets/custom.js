...
jQuery(document).ready(function($){
	...
	"use strict";
		...
		$(document).mouseup(function(e) {
			...
			if ($(e.target).closest(".box-search").length === 0) { 
				searchClose()
			}
		});
		function openCart() {
			$('.box-cart').addClass('show');
			$('.overlay-cart').addClass('open-cart');
			$('body').addClass('overflow-hidden');
		}
		function closeCart() {
			$('.box-cart').removeClass('show');
			$('.overlay-cart').removeClass('open-cart');
	...
	$('.menu-trigger').on('click', function(e){
		...
		if($('.menu-trigger').parents('.header').hasClass('nav-is-visible'))  {
			$('#header').css({ top: 0 }).stop().animate({ top: '88px' }, 200);
		}	
		else {
	...
	$(window).load(function(){
	 ...
	 // animate css + jquery inview configuration
		...
		if( $(".recent-gallery").find("li").length > 1 ) {
			$(".recent-gallery").bxSlider({ auto: true, useCSS:true, pagerCustom: '#bx-pager', autoHover:true, adaptiveHeight:true });
		}		
...
// 	jQuery(document).Dropdown(function($){
...
// 	});
	...
	// 	slidesToShow: 3,
	// 	slidesToScroll: 1,
	// 	asNavFor: '.for-slick-slider',
	// 	dots: true,
	// 	centerMode: true,
	// 	focusOnSelect: true
	//   });
	// $('nav-content-detail').simpleScrollFollow({
	// 	limit_elem: 'content-detail-croll'
	//   });
